...
export const SCAN_LOCATIONS: ScanLocation[] = [
	{
		name: "pi",
		root: process.env.SM_PI_SKILLS ?? join(home, ".pi", "agent", "skills"),
	},
	{
		name: "opencode",
		root:
			process.env.SM_OPENCODE_SKILLS ??
			join(home, ".config", "opencode", "skills"),
	},
	{
		name: "claude",
		root: process.env.SM_CLAUDE_SKILLS ?? join(home, ".claude", "skills"),
	},
	{
		name: "shared",
		root: process.env.SM_SHARED_SKILLS ?? join(home, ".agents", "skills"),
	},
	{
		name: "repo",
		root:
			process.env.SM_REPO_SKILLS ??
			join(home, "Documents", "9. Projects", "agent-skills", "skills"),
		nested: true,
	},
];

export const PORT =
	Number(process.env.SM_PORT) > 0 ? Number(process.env.SM_PORT) : 7788;
