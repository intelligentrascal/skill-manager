...
A skill without a managed canonical copy can be assigned one of three honest origins - verified public GitHub, private/community, or mine/local - with the origin evidence and an append-only history recorded in `skillmgr.yaml`. Public GitHub origins validate the repository and exact `SKILL.md` subpath, then pin a revision; a pasted verified URL needs no assignment reason (the URL is the evidence - reasons stay optional there, while private and local origins keep their attribution/ownership requirements). The dialog pre-fills the subpath from the skill's known location and records the verified skill name from the imported `SKILL.md` on the manifest entry without renaming the stable record key. Private origins reject credential-bearing URLs and require an attribution note. The import previews the selected copy and its hash before creating the canonical baseline, then commits and pushes canonical content plus provenance. A rejected push leaves the verified local commit inspectable for safe retry. Private and local origins never present GitHub stars, owner facts, or pinned revisions.
