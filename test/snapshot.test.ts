...
test("renders a standalone inventory summary", () => {
  const html = renderSnapshot({
    ...
    stats: { totalSkills: 1, totalCopies: 1, drift: 0, duplicate: 0, unique: 1 },
    byName: {
      hallmark: [{ location: "pi", sha: "abc", description: "Make interfaces distinctive." }],
    },
  });
...
test("escapes skill content in a static snapshot", () => {
  const html = renderSnapshot({
    ...
    stats: { totalSkills: 1, totalCopies: 1, drift: 0, duplicate: 0, unique: 1 },
    byName: {
      "<skill>": [{ location: "pi", sha: "abc", description: "<script>alert(1)</script>" }],
    },
  });
